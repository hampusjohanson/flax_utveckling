...
Sub Corr_7()
    ...
    Dim strongestCompetitor As String
    ...
    ' Loop to find specific rows in the CSV (849, 658-663)
    currentRow = 0
    ...
    Do While Not EOF(fileNumber)
        ...
        ' Extract "strongest competitor" from row 849
        If currentRow = 849 Then
            Data = Split(line, ";")
            If UBound(Data) >= 0 Then
                strongestCompetitor = Trim(Data(0))
            Else
                strongestCompetitor = "Unknown"
            End If
            Debug.Print "Strongest competitor from Row 849: " & strongestCompetitor
        End If

        ' Extract table data from rows 658 to 663
        If currentRow >= 658 And currentRow <= 663 Then
            Data = Split(line, ";")
            ...
            For colIndex = 1 To 6
                If UBound(Data) >= colIndex - 1 Then
                    rowData(currentRow - 657, colIndex) = Trim(Data(colIndex - 1))
                Else
                    rowData(currentRow - 657, colIndex) = ""
                End If
            Next colIndex
        End If
    Loop
    ...
    For Each textBox In pptSlide.Shapes
        If textBox.HasTextFrame Then
            If textBox.Name = "Rubrik 2" Then
                textBox.TextFrame.textRange.text = "Correlations to the " & foundTables.count & " strongest associations of " & strongestCompetitor
                Debug.Print "Updated 'Rubrik 2' with: " & strongestCompetitor
                foundTextBox = True
                Exit For
            End If
        End If
    ...
    Debug.Print "Corr_7 completed. Updated " & foundTables.count & " tables."
End Sub
