Attribute VB_Name = "labeling_33"
Option Explicit

Sub IdentifyAndMoveLeftFlankLabels_20()
    Dim sld As slide
    Dim shp As shape
    Dim chrt As chart
    Dim srs As series
            ...
            If chrt.SeriesCollection.count > 0 Then
                ...
                For i = 1 To numPoints
                    For j = 1 To numPoints
                        If i <> j Then
                            If lblLeft(j) < lblLeft(i) And Abs(lblTop(j) - lblTop(i)) < 20 Then ' Check leftward & vertical overlap
...
Sub IdentifyAndMoveRightFlankLabels_20()
    Dim sld As slide
    Dim shp As shape
    Dim chrt As chart
    Dim srs As series
        ...
        If shp.hasChart Then
            ...
            If chrt.SeriesCollection.count > 0 Then
                ...
                For i = 1 To numPoints
                    For j = 1 To numPoints
                        If i <> j Then
                            If lblLeft(j) > lblLeft(i) And Abs(lblTop(j) - lblTop(i)) < 20 Then ' Check rightward & vertical overlap
