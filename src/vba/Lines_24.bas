...
Sub Lines_24()
    ...
    If Not shape Is Nothing And shape.Type = msoTable Then
        Set tbl = shape.table
        Debug.Print "Processing table: Brand_List_1"
        ...
        Select Case visibleCount
            Case 1
                ...
                Debug.Print "Set column 1, rows 2 and 3 to no color for visibleCount = 1"

            Case 2
                ' Set column 1, row 3 to no color
                tbl.cell(3, 1).shape.Fill.visible = msoFalse
                Debug.Print "Set column 1, row 3 to no color for visibleCount = 2"
        End Select

    Else
        Debug.Print "Table 'Brand_List_1' not found. Skipping."
    End If
End Sub
