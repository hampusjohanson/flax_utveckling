...
Sub Corr_Bold_4()
    ...
    ' If fewer than 4 tables are found, exit
    If tblCount < 4 Then
        MsgBox "Not enough tables found on the slide (minimum 4 required).", vbExclamation
        Exit Sub
    End If
    ...
    ' Apply bold formatting if text appears in at least 4 out of ALL tables
    For Each tbl In tbls
        For rowIndex = 1 To tbl.Rows.count
            On Error Resume Next
            textValue = Trim(tbl.cell(rowIndex, 2).shape.TextFrame.textRange.text)
            ...
            If textValue <> "" Then
                ' Check if the text has at least 4 occurrences across all tables
                For i = 1 To uniqueCount
                    If occurrences(i, 1) = textValue Then
                        If occurrences(i, 2) >= 4 Then
