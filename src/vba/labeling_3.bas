Attribute VB_Name = "labeling_3"
Option Explicit

Sub IdentifyAndMoveLeftFlankLabels()
    Dim sld As slide
    Dim shp As shape
    Dim chrt As chart
    Dim srs As series
...
Sub IdentifyAndMoveRightFlankLabels()
    Dim sld As slide
    Dim shp As shape
    Dim chrt As chart
    Dim srs As series
        ...
        If shp.hasChart Then
            ...
            If chrt.SeriesCollection.count > 0 Then
                ...
                For i = 1 To numPoints
                    For j = 1 To numPoints
                        If i <> j Then
                            If lblLeft(j) > lblLeft(i) And Abs(lblTop(j) - lblTop(i)) < 15 Then ' Check rightward & vertical overlap
