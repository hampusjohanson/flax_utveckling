Attribute VB_Name = "AA_Count_boxes_series_A8"

Sub AA_Count_boxes_series_A8()
    ...
    ' === Ensure there are at least 8 data points in both series ===
    If series1 Is Nothing Or series2 Is Nothing Then Exit Sub
    If series1.Points.count < 8 Or series2.Points.count < 8 Then Exit Sub

    ' === Extract the eighth value from each series ===
    val1 = series1.values(8)
    val2 = series2.values(8)
    ...
    For Each shape In pptSlide.Shapes
        If shape.Name = "Leftie_8" Then
            Set leftieBox = shape
            Exit For
        End If
    Next shape

    ' === Exit if Leftie_8 was not found ===
    If leftieBox Is Nothing Then Exit Sub

    ' === Paste the percentage into Leftie_8 ===
