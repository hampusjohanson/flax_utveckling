Attribute VB_Name = "AA_Count_boxes_series_A6"
Sub AA_Count_boxes_series_A6()
    ...
    ' === Ensure there are at least 6 data points in both series ===
    If series1 Is Nothing Or series2 Is Nothing Then Exit Sub
    If series1.Points.count < 6 Or series2.Points.count < 6 Then Exit Sub

    ' === Extract the sixth value from each series ===
    val1 = series1.values(6)
    val2 = series2.values(6)
    ...
    For Each shape In pptSlide.Shapes
        If shape.Name = "Leftie_6" Then
            Set leftieBox = shape
            Exit For
        End If
    Next shape

    ' === Exit if Leftie_6 was not found ===
    If leftieBox Is Nothing Then Exit Sub

    ' === Paste the percentage into Leftie_6 ===
