Attribute VB_Name = "ToppA_3"
' Macro: Create a new table named 'border_table_weaker', select it, and apply bottom borders to all cells
Sub ToppA_3()
    ...
    ' Calculate the row count for the new table (Weaker row count + 1)
    rowCount = weakerTable.Rows.count - 1

    ' Create the new table
    Dim shape As shape
    Set shape = currentSlide.Shapes.AddTable(rowCount, 1, positionLeft, positionTop, columnWidth, firstRowHeight)
    shape.Name = "border_table_weaker"
    ...
    ' Apply bottom borders specifically to border_table_weaker
    Dim selectedTable As table
    Set selectedTable = currentSlide.Shapes("border_table_weaker").table
