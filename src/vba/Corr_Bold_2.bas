...
Sub Corr_Bold_2()
    ...
    ' If fewer than 2 tables are found, exit
    If tblCount < 2 Then
    ...
    ' Apply bold formatting if text appears in at least 2 out of ALL tables
    For Each tbl In tbls
        For rowIndex = 1 To tbl.Rows.count
            On Error Resume Next
            textValue = Trim(tbl.cell(rowIndex, 2).shape.TextFrame.textRange.text)
            ...
            If textValue <> "" Then
                ' Check if the text has at least 2 occurrences across all tables
                For i = 1 To uniqueCount
                    If occurrences(i, 1) = textValue Then
                        If occurrences(i, 2) >= 2 Then
