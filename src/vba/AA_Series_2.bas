...
Sub AA_Series_2()
    ...
    For rowIndex = 1 To tableShape.table.Rows.count
        ...
        ' If the value is "yes", include corresponding C-column value from the chart
        If currentCellValue = "yes" Then
            yValues.Add "C" & (rowIndex + 1) ' Store row index for later retrieval
        End If
    Next rowIndex
    ...
    newSeries.Name = "Serie2" & chartShape.chart.SeriesCollection.count
    ...
    With newSeries.Format.Fill
        .visible = msoTrue
        .ForeColor.RGB = RGB(112, 113, 140) ' Hex #70718C
        .Solid
    End With
