...
Sub Corr_6()
    ...
    ' Loop to read data from rows 652–657
    currentRow = 0
    Do While Not EOF(fileNumber)
        Line Input #fileNumber, line
        currentRow = currentRow + 1

        If currentRow >= 652 And currentRow <= 657 Then
            Data = Split(line, ";")
            ...
            For colIndex = 1 To 6
                If UBound(Data) >= colIndex - 1 Then
                    rowData(currentRow - 651, colIndex) = Trim(Data(colIndex - 1))
                Else
                    rowData(currentRow - 651, colIndex) = ""
                End If
            Next colIndex
        End If
    Loop
