...
Sub Corr_Bold_3()
    ...
    ' If fewer than 3 tables are found, exit
    If tblCount < 3 Then
        MsgBox "Not enough tables found on the slide.", vbExclamation
        Exit Sub
    End If

    Debug.Print "Total tables found: " & tblCount
    
    ' Scan column 2 of each table and store occurrences in array
            ...
            If textValue <> "" Then
                ...
                For i = 1 To uniqueCount
                    If occurrences(i, 1) = textValue Then
                        ' If found, increment the occurrence count
                ...
                ' If textValue is not found, add it to the array
                If Not found Then
    ...
    ' Apply bold formatting if text appears in at least 3 out of ALL tables
    For Each tbl In tbls
        For rowIndex = 1 To tbl.Rows.count
            On Error Resume Next
            textValue = Trim(tbl.cell(rowIndex, 2).shape.TextFrame.textRange.text)
            ...
            If textValue <> "" Then
                ' Check if the text has at least 3 occurrences across all tables
                For i = 1 To uniqueCount
                    If occurrences(i, 1) = textValue Then
                        If occurrences(i, 2) >= 3 Then
