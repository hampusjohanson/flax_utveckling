...
Sub Lines_23()
    ...
    If Not shape Is Nothing And shape.Type = msoTable Then
        Set tbl = shape.table
        Debug.Print "Processing table: Brand_List_2"

        Select Case visibleCount
            Case 1, 2, 3
                ' Set column 1, rows 1, 2, and 3 to no color
                tbl.cell(1, 1).shape.Fill.visible = msoFalse
                tbl.cell(2, 1).shape.Fill.visible = msoFalse
                tbl.cell(3, 1).shape.Fill.visible = msoFalse
                Debug.Print "Set column 1, rows 1-3 to no color for visibleCount = " & visibleCount

            Case 4
                ' Set column 1, rows 2 and 3 to no color
                tbl.cell(2, 1).shape.Fill.visible = msoFalse
                tbl.cell(3, 1).shape.Fill.visible = msoFalse
                Debug.Print "Set column 1, rows 2-3 to no color for visibleCount = 4"

            Case 5
                ' Set column 1, row 3 to no color
                tbl.cell(3, 1).shape.Fill.visible = msoFalse
                Debug.Print "Set column 1, row 3 to no color for visibleCount = 5"
        End Select

    Else
        Debug.Print "Table 'Brand_List_2' not found. Skipping."
    End If
End Sub
